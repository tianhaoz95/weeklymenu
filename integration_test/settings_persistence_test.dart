...
import 'package:weeklymenu/data/models/user_model.dart'; // Import UserModel
...
void main() {
  ...
  group('Settings Persistence Test', () {
    testWidgets('Selected meal types and weekdays should persist',
      ...
      app.main();
      await tester.pumpAndSettle(const Duration(seconds: 5)); // Increased duration
      ...
      await tester.pumpAndSettle(const Duration(seconds: 5)); // Added duration after login
      ...
      await settingsRepository.saveSettings(
          userId, SettingsModel(id: userId)); // Corrected parameter name

      // Assuming initial location is login, and it redirects to weekly-menu
      await tester.pumpAndSettle(const Duration(seconds: 5)); // Increased duration
      await tester.pumpAndSettle(const Duration(seconds: 5)); // Additional duration for UI to settle

      // Assert that the BottomNavigationBar is present
      expect(find.byType(BottomNavigationBar), findsOneWidget, reason: 'BottomNavigationBar should be present after login redirection');
      ...
      await tester.tap(find.ancestor(of: breakfastFinder, matching: find.byType(FilterChip)));
      await tester.pumpAndSettle();

      FilterChip currentBreakfastChip = tester.widget(
          find.ancestor(of: breakfastFinder, matching: find.byType(FilterChip)));
      expect(currentBreakfastChip.selected, !initialBreakfastSelected,
          reason: 'Breakfast FilterChip should have toggled its selected state after tap');
      ...
      await tester.tap(find.ancestor(of: mondayFinder, matching: find.byType(FilterChip)));
      await tester.pumpAndSettle();

      FilterChip currentMondayChip = tester.widget(
          find.ancestor(of: mondayFinder, matching: find.byType(FilterChip)));
      expect(currentMondayChip.selected, !initialMondaySelected,
          reason: 'Monday FilterChip should have toggled its selected state after tap');
      ...
      FilterChip persistedBreakfastChip = tester.widget(
          find.ancestor(of: breakfastFinder, matching: find.byType(FilterChip)));
      expect(persistedBreakfastChip.selected, !initialBreakfastSelected,
          reason: 'Breakfast FilterChip should have its toggled state persisted');

      FilterChip persistedMondayChip = tester.widget(
          find.ancestor(of: mondayFinder, matching: find.byType(FilterChip)));
      expect(persistedMondayChip.selected, !initialMondaySelected,
